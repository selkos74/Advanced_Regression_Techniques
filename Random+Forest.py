...
X_testRF = finaldata.loc['test']
X_trainRF = finaldata.loc['train']
...
m = X_trainRF.mean() #Prosečna vrednost parametra u traingn set-u
...
#Interquartile range
def IQR(X_trainRF):
    return np.percentile(X_trainRF, 75) - np.percentile(X_trainRF, 25)


X_trainRF = (X_trainRF - m) / IQR(X_trainRF)
X_testRF = (X_testRF - m) / IQR(X_trainRF)
...
x_trainRF, x_testRF, y_trainRF, y_testRF = train_test_split(X_trainRF, Y_trainRF, test_size=0.25, random_state=42)
...
#Treniramo model Random Forest korišćenjem training set-a
forest_model.fit(X_trainRF, Y_trainRF)
...
melb_preds = forest_model.predict(x_testRF)


# In[49]:


diff = pd.DataFrame(melb_preds - y_testRF)
...
for f in range(X_trainRF.shape[1]):
...
plt.xticks(range(X_trainRF.shape[1]), indices)
...
pd.Series(X_trainRF.columns)


# In[61]:


importance_by_feature = pd.concat([pd.Series(X_trainRF.columns), pd.Series(forest_model.feature_importances_)], axis=1)
importance_by_feature.columns = ["Atribute", "Importance Value"]
importance_by_feature.sort_values(
    "Importance Value", ascending=False).head(10).to_excel("Random_Forest_feature_importance.xlsx", 
...
x_trainRF, x_testRF, y_trainRF, y_testRF
...
final_rf = RandomForestRegressor(
    n_estimators=300, min_samples_split=8, min_samples_leaf=3, max_features=3, max_depth=100, bootstrap=True
)
...
final_rf.fit(X_trainRF, Y_trainRF)
...
importance_by_feature = pd.concat([pd.Series(X_trainRF.columns), pd.Series(final_rf.feature_importances_)], axis=1)
importance_by_feature.columns = ["Atribute", "Importance Value"]
# importance_by_feature.sort_values(
#     "Importance Value", ascending=False).head(10).to_excel("Random_Forest_feature_importance.xlsx", 
#                                                            header=True, index=False)
...
importance_by_feature.sort_values(
        "Importance Value", ascending=False
    ).head(10).to_excel(
        "Random_Forest_feature_importance_after_parameters_tuning.xlsx", header=True, index=False
    )
...
final_melb_preds = final_rf.predict(x_testRF)
...
diff = pd.DataFrame(final_melb_preds - y_testRF)
diff.columns = ["diff"]
diff["squared"] = diff["diff"].apply(lambda x: x*x)
diff.head()
...
np.sqrt(diff["squared"].sum()/365)


# In[85]:


diff.sort_values("squared", ascending=False).head()
...
r = r2_score(y_testRF, final_melb_preds) # 0.871 R-squared 0.802 Adj R-squared
...
adj_r = 1 - (1-0.871)*(365 - 1)/(365 - 127 - 1)
